...
for x in utt2spk spk2utt feats.scp text segments wav.scp cmvn.scp vad.scp \
    reco2file_and_channel spk2gender utt2lang utt2score utt2_new_word_score utt2_new_utt_score utt2IntLabel utt2uniq utt2dur reco2dur utt2num_frames; do
  if [ -f $data/$x ]; then
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang utt2score utt2_new_word_score utt2_new_utt_score utt2IntLabel $maybe_wav $maybe_utt2dur $maybe_utt2num_frames; do
    if [ -f $data/$x ]; then
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2score utt2_new_word_score utt2_new_utt_score utt2IntLabel utt2dur utt2num_frames $maybe_wav $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
